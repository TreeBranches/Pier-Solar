...
If the latter is the case, then the slight variances in the speed of the character is enough to throw the script off randomly at randomly times, which is what's happening during my tests - parts of the script which had previously been conquered perfectly and can consistently get through without trouble, will suddenly, randomly start to fail, and it will not happen at the same points. There must be some kind of 'give' to the controls, as I can very frequently get past the halfway point of the map (where there's lots of 'stairs' after a big horizontal stretch, but it is enough to make sure that this script is not 100% reliable.
